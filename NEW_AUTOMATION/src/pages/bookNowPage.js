...
class BookNowPage extends BasePage {
  ...
  async expectCurrencyMatchesDestination(destinationInput, timeout = 25_000) {
    ...
    while (Date.now() < deadline) {
      ...
      if (lastCodes.some((code) => code === dest.currency)) {
        console.log(
          `[book-now] Currency OK for ${dest.code}: ${dest.currency} (amount ignored)`,
        );
        return;
      }
      await this.settle(500);
    ...
    throw new Error(
      `Expected currency code ${dest.currency} for ${dest.code}, got: ${
