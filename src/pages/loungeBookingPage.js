...
class LoungeBookingPage extends BasePage {
  ...
  async clickGetPriceLeavingDefaults() {
    ...
    // Codegen TC06: getByRole('button', { name: 'Get Price' }).click()
    const getPrice = this.page.getByRole('button', { name: 'Get Price' }).first();
    const reserve = this.page.getByRole('button', { name: 'Reserve Now' }).first();
    await expect(getPrice).toBeAttached({ timeout: 60_000 });
    for (let attempt = 1; attempt <= 3; attempt++) {
      await this.dismissBlockingOverlays();
      const clicked = await getPrice
      ...
      console.log(`[booking] Get Price click attempt ${attempt}`);
      if (await reserve.isVisible({ timeout: attempt === 3 ? 45_000 : 15_000 }).catch(() => false)) {
        console.log('[booking] Get Price clicked — Reserve Now visible');
        return;
      }
    }
    await expect(reserve).toBeVisible({ timeout: 15_000 });
  ...
  async addShowerThirtyMinsAddon() {
    ...
    await this.dismissBlockingOverlays();
    const addBtn = this.showerThirtyMinsAddButton();
    await addBtn.scrollIntoViewIfNeeded().catch(() => {});
    await expect(addBtn).toBeVisible({ timeout: 30_000 });
    await this.waitBeforeTransition();
    await addBtn.click({ force: true });
    console.log('[booking] Clicked Shower 30 mins Add (div.service-CTA a.add-addon)');
    ...
    await this.settle(1_000);
    await this.dismissBlockingOverlays();
    if (await this.bookingSummaryCheckOutButton().isVisible({ timeout: 5_000 }).catch(() => false)) {
      console.log('[booking] Shower 30 mins added — booking-summary Check Out visible');
      return;
    }
  ...
  async clickReserveNow() {
    if (this.isMobile()) {
      ...
      const postReserveReady = this.bookingSummaryCheckOutButton()
    ...
    if (await checkOut.isVisible({ timeout: 5_000 }).catch(() => false)) {
      console.log('[booking] Check Out already visible after Reserve Now');
      return;
    }

    // Codegen TC06: getByRole('button', { name: 'Reserve Now' }).click()
    const reserve = this.page.getByRole('button', { name: 'Reserve Now' }).first();
    ...
    console.log('[booking] Reserve Now clicked');
    await expect(checkOut).toBeVisible({ timeout: 90_000 });
    console.log('[booking] Check Out is visible after Reserve Now');
  }
  ...
  async clickCheckOut() {
    ...
    if (this.isMobile()) {
      ...
      if (await summaryCheckOut.isVisible({ timeout: 8_000 }).catch(() => false)) {
        console.log('[checkout] Clicking booking-summary Check Out (button.reserve-now-btn) — not Reserve Now');
        await summaryCheckOut.click({ force: true });
      } else {
        await expect(this.page.getByRole('button', { name: /^Check Out$/i })).toBeVisible({
          timeout: 90_000,
        });
        await this.page.getByRole('button', { name: /^Check Out$/i }).click();
      }
    }
