...
class GuestCheckoutPage extends BasePage {
  ...
  async acceptCheckoutRadiosAndTerms({ expectButton, clickLabels = true } = {}) {
    ...
    const confirmOrPayment = this.isMobile()
      ? this.page
          .getByRole('button', { name: /Confirm & Proceed/i })
          .or(this.page.getByRole('link', { name: /Confirm & Proceed/i }))
          .or(
            this.page
              .locator('#guestcheckoutbutton, button.fullWidth.reserve-now-btn, button.reserve-now-btn')
              .filter({ hasText: /Confirm\s*&\s*Proceed|Payment/i }),
          )
  ...
  checkoutConfirmAndProceedButton() {
    return this.page
      .getByRole('button', { name: /Confirm & Proceed/i })
      .or(this.page.getByRole('link', { name: /Confirm & Proceed/i }))
      .or(
  ...
  async clickConfirmAndProceed() {
    ...
    if (this.isMobile()) {
      ...
      const confirmBtn = this.checkoutConfirmAndProceedButton();
      await expect(confirmBtn).toBeVisible({ timeout: 30_000 });
      console.log('[checkout] Mobile: checkout Confirm & Proceed after form/email');
      await this.#navigateToPayment(() => confirmBtn.click({ force: true }));
      return;
    }
  ...
  async #clickGuestPaymentButton() {
    ...
    const candidates = [
      ...(this.isMobile() ? [this.checkoutConfirmAndProceedButton()] : []),
