...
class LmsPage extends BasePage {
  ...
  #outletSearchQueries(number, { propertyName = '', locationText = '', gate = '' } = {}) {
    ...
    if (/Plaza Premium First/i.test(prop) || /\bPPF\b/i.test(combined)) {
      queries.push('Plaza Premium First', 'PPF');
    } else if (/Plaza Premium Lounge/i.test(prop)) {
      queries.push('Plaza Premium Lounge');
    }

    // Hall names only when AMS/location actually says so — not as a PPL default.
  ...
  #outletHintForGate(gate, locationText = '') {
    ...
    if (/East Hall/i.test(locationText) || gateNum === '1') return 'East Hall';
  ...
  async openBookingsAndPrepare() {
    ...
    await this.#ensureShowApprovedEnabled();
  }
  ...
  /**
   * TC06/TC07 Bookings setup: stay on Bookings, keep the captured outlet, Show Approved ON.
   */
  async ensureOnBookingsWithOutlet(title) {
    await this.ensureOnBookings();
    const wanted = String(title || '').replace(/\s+/g, ' ').trim();
    if (!wanted) throw new Error('Captured LMS outlet name is required before Bookings search');
    const current = await this.#currentBookingsOutletLabel();
    if (current !== wanted) {
      console.log(`[lms] Bookings outlet "${current || '(empty)'}" — selecting captured "${wanted}"`);
      await this.selectOutletByTitle(wanted);
    } else {
      console.log(`[lms] Bookings outlet already selected: ${current}`);
    }
    await this.#ensureShowApprovedEnabled();
  }
  ...
  async #ensureAllBookingsView() {
    ...
    const showApproved = this.page.locator('#customSwitch1, input[name="showApproved"]').first();
    if ((await showApproved.count().catch(() => 0)) > 0) {
      console.log('[lms] Not turning off Default view — Show Approved (#customSwitch1) must stay ON for search');
      return;
    }
    ...
    const switchCtl = label
      ...
      .first();
    const isShowApproved = await switchCtl
      .evaluate((el) => {
        const input = el.matches('input')
          ? el
          : el.querySelector('#customSwitch1, input[name="showApproved"]');
        const target = input || el;
        return target.id === 'customSwitch1' || target.getAttribute('name') === 'showApproved';
      })
      .catch(() => false);
    if (isShowApproved) {
      console.log('[lms] Leaving Default view on — it is Show Approved (#customSwitch1)');
      return;
    }
  ...
  async #ensureShowApprovedEnabled() {
    const checkbox = this.page.locator('#customSwitch1, input[name="showApproved"]').first();
    await checkbox.waitFor({ state: 'attached', timeout: 15_000 }).catch(() => null);
    if ((await checkbox.count().catch(() => 0)) === 0) {
      throw new Error('Show Approved (#customSwitch1) was not found before LMS booking search');
    }

    const turnOn = async () => {
      const label = this.page.locator('label[for="customSwitch1"], .custom-control-label[for="customSwitch1"]').first();
    ...
    if (!(await checkbox.isChecked().catch(() => false))) {
      console.log('[lms] Show Approved (#customSwitch1) is OFF — turning it ON before search');
      await turnOn();
      await this.#waitForFetchingBookingsGone();
    }

    await expect(checkbox).toBeChecked({ timeout: 10_000 });
    console.log('[lms] Show Approved (#customSwitch1) is ON before search');
  }

  async #prepareBookingSearchBox() {
    await this.ensureOnBookings();
    await this.#waitForFetchingBookingsGone();
    await this.#dismissOverlays();
    await this.page.keyboard.press('Escape').catch(() => {});
  ...
  async searchBookingId(orderNo) {
    ...
    const search = await this.#prepareBookingSearchBox();
  ...
  async #bookingRowVisible(id, timeout = 8_000) {
    return this.bookingNumberCell(id).isVisible({ timeout }).catch(() => false);
  }

  async #pasteBookingIdAndPressEnter(id) {
    const search = await this.#prepareBookingSearchBox();
    await search.click({ force: true, timeout: 5_000 });
    ...
    if (!pasted || (await search.inputValue().catch(() => '')) !== id) {
      await search.fill(id);
    }
    await expect(search).toHaveValue(id, { timeout: 5_000 });
    await search.press('Enter');
    await this.page.keyboard.press('Enter').catch(() => {});
    console.log(`[lms] Pasted booking id and pressed Enter: ${id}`);
    await this.#waitForFetchingBookingsGone();
    await this.#dismissOverlays();
  }

  async #reloadBookingsKeepOutlet(outletTitle) {
    await this.page.reload({ waitUntil: 'domcontentloaded' });
    console.log('[lms] Wait 5s after full page refresh');
    await this.page.waitForTimeout(5_000);
    await this.#dismissOverlays();
    await this.ensureOnBookings();
    await this.#ensureAllBookingsView();
    const after = await this.#currentBookingsOutletLabel();
    const wanted = String(outletTitle || '').replace(/\s+/g, ' ').trim();
    if (wanted && after !== wanted) {
      console.log(`[lms] Outlet reset after refresh ("${after || '(empty)'}") — re-selecting ${wanted}`);
      await this.selectOutletByTitle(wanted);
    }
    await this.waitForBookingsTable();
    await this.#ensureShowApprovedEnabled();
  ...
  async #searchBookingUntilVisible(id, outletTitle, maxRefreshes = 3) {
    await this.#pasteBookingIdAndPressEnter(id);
    if (await this.#bookingRowVisible(id, 10_000)) {
      console.log(`[lms] Captured booking visible in LMS table: ${id}`);
      return this.bookingNumberCell(id);
    }

    for (let attempt = 1; attempt <= maxRefreshes; attempt++) {
      console.log(`[lms] No booking row — full page refresh ${attempt}/${maxRefreshes}`);
      await this.#reloadBookingsKeepOutlet(outletTitle);
      await this.#pasteBookingIdAndPressEnter(id);
      if (await this.#bookingRowVisible(id, 10_000)) {
        console.log(`[lms] Captured booking visible after refresh ${attempt}/${maxRefreshes}: ${id}`);
        return this.bookingNumberCell(id);
      }
    }

    throw new Error(
      `Booking ${id} not found in LMS Bookings after ${maxRefreshes} full-page refresh retries`,
    );
  }
  ...
  async #waitForSingleBookingResult(orderNo, timeout = 45_000) {
    ...
    if (await this.#bookingRowVisible(id, 12_000)) {
      console.log(
    ...
    const outlet = await this.#currentBookingsOutletLabel();
    return this.#searchBookingUntilVisible(id, outlet, 3);
  }

  async searchAndValidateBooking(orderNo, timeout = 60_000, expectedOutlet = '') {
    return this.refreshBookingsThenSearch(orderNo, timeout, expectedOutlet);
  }

  async refreshBookingsThenSearch(orderNo, timeout = 60_000, expectedOutlet = '') {
    await this.waitForBookingsTable();
    const wanted = String(expectedOutlet || '').replace(/\s+/g, ' ').trim();
    if (wanted) {
      const current = await this.#currentBookingsOutletLabel();
      if (current !== wanted) {
        console.log(`[lms] Bookings outlet "${current || '(empty)'}" before search — selecting "${wanted}"`);
        await this.selectOutletByTitle(wanted);
      }
    }
    await this.#ensureAllBookingsView();
    await this.#ensureShowApprovedEnabled();
    const outlet = wanted || (await this.#currentBookingsOutletLabel());
    const id = String(orderNo || '').trim();
    console.log(
      `[lms] Outlet confirmed on Bookings${outlet ? ` (${outlet})` : ''} — searching captured booking id: ${id}`,
    );

    await this.searchBookingId(id);
    if (await this.#bookingRowVisible(id, 12_000)) {
      console.log(`[lms] Captured booking visible in LMS table: ${id}`);
      return this.bookingNumberCell(id);
    }
  ...
  async verifyCapturedBookingInLms(
    ...
    if (airport !== 'HKG') {
      ...
      for (let i = 1; i <= attempts; i++) {
        try {
          await this.refreshBookingsThenSearch(orderNo, i === attempts ? 45_000 : 20_000);
          return;
        } catch (err) {
    ...
    for (const outlet of outlets) {
      ...
      try {
        await this.refreshBookingsThenSearch(orderNo, 60_000);
        return;
      } catch (err) {
        lastErr = err;
        const preview = await this.#bookingTablePreview();
        console.log(
          `[lms] Search/validate ${outlet} missed ${orderNo}. Table: ${preview || '(empty)'}`,
        );
      }
    }
